...
class LS_LoadBenModel:
    ...
    @classmethod
    def INPUT_TYPES(s):
        model_list = list(get_models().keys())
    ...
    FUNCTION = "load_ben_model"
    CATEGORY = '😺dzNodes/LayerMask'

    def load_ben_model(self, model, device):
        model_path = get_models()[model]
        b_model = BEN_Base().to(device).eval()
        b_model.loadcheckpoints(model_path)

        return ({"model":b_model, "device":device, "name":model},)


class LS_BenUltra:

    def __init__(self):
        self.NODE_NAME = 'BenUltra'
    ...
    FUNCTION = "ben_ultra_v2"
    CATEGORY = '😺dzNodes/LayerMask'

    def ben_ultra_v2(self, ben_model, image, detail_method, detail_erode, detail_dilate,
                       black_point, white_point, max_megapixels, process_detail):
        ret_images = []
        ret_masks = []

        b_model = ben_model['model']
        device= ben_model['device']
        ...
        for i in image:
            ...
            mask, _ = b_model.inference(orig_image)
            _mask = image2mask(mask)
...
NODE_CLASS_MAPPINGS = {
    "LayerMask: BenUltra": LS_BenUltra,
    "LayerMask: LoadBenModel": LS_LoadBenModel,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "LayerMask: BenUltra": "LayerMask: Ben Ultra(Advance)",
    "LayerMask: LoadBenModel": "LayerMask: Load Ben Model(Advance)",
}
