...
from .imagefunc import log, tensor2pil, pil2tensor, image2mask, mask2image, mask_fix, RMBG, chop_image
...
class MaskByDifferent:
    ...
    def __init__(self):
        self.NODE_NAME = 'MaskByDifferent'
    ...
    def mask_by_different(self, image_1, image_2, gain, fix_gap, fix_threshold, main_subject_detect):
        ...
        if tensor2pil(image1s[0]).size != tensor2pil(image2s[0]).size:
            log(f"Error: {self.NODE_NAME} skipped, because the image size is not match.", message_type='error')
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "LayerMask: MaskByDifferent": "LayerMask: MaskByDifferent(Advance)"
}